...
description: "Complete Ethereum JSON-RPC client with batch processing, subscriptions, and transaction signing"
nav_order: 8
---
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = EthJsonrpcClientConfig {
        endpoints: vec![
            Endpoint::new(
                "https://eth-rpc-url-goes-here.com".into(),
                "path_if_any".into()
            )
        ],
...
async def main():
    ...
    config = EthJsonrpcClientConfig(
        endpoints=[
            Endpoint("https://eth-rpc-url-goes-here.com", "path_if_any")
        ],
...
async function main() {
    ...
    const config = new EthJsonrpcClientConfig({
        endpoints: [
            new Endpoint("https://eth-rpc-url-goes-here.com", "path_if_any")
        ],
...
async def main():
    client = EthJsonrpcClient(endpoints=["https://eth-rpc-url-goes-here.com"])
    await client.connect()
...
async function main() {
    const client = new EthJsonrpcClient({ endpoints: ['https://eth-rpc-url-goes-here.com'] });
    await client.connect();
...
async def main():
    ...
    config = EthJsonrpcClientConfig(
        endpoints=[
            Endpoint("https://eth-rpc-url-goes-here.com", "")
        ],
        http_transport_config=http_config,
...
async function main() {
    ...
    const config = new EthJsonrpcClientConfig({
        endpoints: [
            new Endpoint("https://eth-rpc-url-goes-here.com", "")
        ],
